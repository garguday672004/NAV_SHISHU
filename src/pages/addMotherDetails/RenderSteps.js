...
const RenderSteps = () => {
  const { step } = useSelector((state) => state.surrogate);
  console.log(step);
  ...
  const steps = [
    ...
    {
      id: 3,
      title: "Agreement Details & Submit",
    },
  ];
  ...
  return (
    <div>
      ...
      {step === 1 && <SurrogateProfileForm />}
      {step === 2 && <HealthDetailsForm />}
      {step === 3 && <AgreementDetailsForm />}
    </div>
  );
};
